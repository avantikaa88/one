...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $adoption_fee = isset($_POST['adoption_fee']) && $_POST['adoption_fee'] !== '' ? (float)$_POST['adoption_fee'] : 0;
    ...
    if ($adoption_fee < 0) $errors[] = "Invalid adoption fee."; // only negative blocked
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute()) {
            ...
            $_POST = [];       // clear all form data
            $type_option = ''; // reset type selection
            $image_path = '';
        } else {
...
<form id="add-pet-form" method="POST" enctype="multipart/form-data">
    ...
    <input type="number" step="0.01" name="adoption_fee" value="<?= $_POST['adoption_fee'] ?? '' ?>" placeholder="0.00">
