...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $allowed_types = ['user', 'admin', 'vet', 'pet_shop'];
    if (!in_array($user_type, $allowed_types)) $errors[] = "Invalid user type selected";
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("UPDATE users SET name=?, user_name=?, email=?, phone=?, gender=?, address=?, user_type=? WHERE user_id=?");
        $stmt->bind_param("sssssssi", $name, $user_name, $email, $phone, $gender, $address, $user_type, $user_id);
...
.back-link { display:inline-block; margin-bottom:20px; color:#00a8ff; text-decoration:none; }
...
<form method="post">
    ...
    <select name="user_type" required>
        ...
        <option value="vet" <?= $user['user_type']=='vet'?'selected':'' ?>>Veterinarian</option>
        <option value="pet_shop" <?= $user['user_type']=='pet_shop'?'selected':'' ?>>Pet Shop</option>
    </select>
...
<p style="margin-top:20px; color:#666;">
    <strong>Note:</strong> Changing user type may affect their dashboard access.
</p>
</div>
